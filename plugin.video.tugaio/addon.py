#!/usr/bin/python
#Agredecimentos ao Enen92 e Manfarricos


import urllib,urllib2,re,xbmcplugin,xbmcgui,xbmcaddon,xbmc,os
from bs4 import BeautifulSoup
from resources.lib import Downloader #Enen92 class, muchas gracias :D
...
def categorias():
	if getSetting("pref_site") == 'Geral' or getSetting("pref_site") == 'Ambos':
		addDir('Filmes', site, 1, os.path.join(artfolder,'filmes.png'), 0)
		addDir('Series', site, 2, os.path.join(artfolder,'series.png'), 0)
	...
	if getSetting("pref_site") == 'Kids' or getSetting("pref_site") == 'Ambos':
		addDir('Filmes KIDS', sitekids, 1, os.path.join(artfolder,'filmes_kids.png'), 0)
...
def getSeasons(url):
	...
	for temporada in soup.findAll('h2'):
		addDirSeason(temporada.text, url, 5, os.path.join(artfolder,'temporadas','temporada'+str(temporadaN+1)+'.png'), 0, temporadaN)
		temporadaN+=1
	#setVista('temporadas')
	vista_temporadas()
